...
(SCL diode)  (~149 keV)  (2 prebunchers + 3 solenoids,  (~21 MeV captured,  (sections 2–8,
              velocity bunching + focusing, ~2 m)        SLAC TW section)     ~305 MeV, Impact-T)
```
...
transmission is **~64%** on the energy-flat ~150 keV zc beam; an optional re-optimization of the
...
~21 MeV exit beam and accelerates the relativistic core on-crest to ≈305 MeV at the default
...
- **Stage API:** each stage package (`cathode/`, `gun/`, `injector/`, `linac_sec1/`, `linac_rest/`) is a top-level facade. `import cathode; cathode.run()` (likewise for `gun`, `injector`, `linac_sec1`, `linac_rest`) builds the field map (if any), runs the sim, and generates that stage's plots. Each exposes `run(plots=True)`, `plot()` (figures from existing diags), and `config(**kwargs)`. **The four WarpX stages no longer take their constants from `config()`** — those live in `<stage>.yaml` (operating point + Balanced profile baked in); retune by editing the YAML. `config()` remains the knob API only for **`linac_rest`** (the Impact-T stage, `POWER_MW`/`Np`/`Ntstep`/`QUADS_ON`/`SPACE_CHARGE`). The four WarpX stages route through `pipeline/_runner.py` (`Stage`, subprocess-isolated sim) and drive lume-warpx in-sim; `linac_rest` routes through `pipeline/_impact_runner.py` (`ImpactStage`, in-process — see Project Architecture).
- **Performance knobs:** for the four WarpX stages the runtime-critical parameters live in `<stage>.yaml` — grid (`number_of_cells`) and solver (`required_precision`, `maximum_iterations`) in the WarpX sections, and step/macroparticle/dump knobs (`CFL`/`TRANSIT_MARGIN`/`AVG_SPEED_FRAC`/`MAX_STEPS`, `MAX_PART`, `N_DIAGS`, `DIAG_PERIOD`) in the `params:` block. The shipped YAML defaults are the **Balanced** profile (cathode `PPC=6`/`required_precision=3e-5`; gun `nz=384`/`MAX_PART=50000`/`1e-4`; injector `CFL=0.95`/`maximum_iterations=150`/`1e-3`); edit the YAML for a Conservative/Aggressive point. (`linac_rest`'s knobs stay on `config()`.) The **injector dominates total runtime** (its self-field MLMG solve over the ~2 m long-thin box). The gun's cells are near-isotropic, so it scales ≈ `nz²` (per-step cost ∝ cells, and `dz = zmax/nz` ⇒ fewer steps as `nz` drops) — coarsening `nz` is a big clean win. The **injector is the opposite** (measured): its long-thin box (2.10 m × 36 mm) has anisotropic cells and a convergence-bound MLMG solve, so coarsening `NZ` *slows* the per-step solve faster than it removes cells and under-resolves the ~1 mm bunch — keep `NZ=1664` (dz≈1.26 mm ⇒ 2.80:1 aspect at `NR=80`) and speed it via `CFL` (fewer steps) and `MAX_ITERS`/`REQUIRED_PRECISION` (cheaper solve) instead. The injector run is **convergence-bound, so its cost over the longer 2.10 m box rose >2×** vs the old 1.30 m prebuncher (~60 s vs ~24 s); the linac got cheaper (dropped the solenoid + heavy radial scrape), partly offsetting. Lowering knobs trades accuracy for speed. The `linac_sec1` stage is a single ~3.5 m RZ run (≈45 s at the default `NZ=1664`, `NR=16` over the 9.547 mm bore → ~3.5:1 cells); like the injector it is a long-thin box, so its cells must stay near the ≈3:1 rule or the MLMG self-field solve diverges (`MLMG failed`).
...
- **Injector power/phase scan:** the operating point now lives in `injector/injector.yaml` (`params:` `PREB1_KW`/`PREB2_KW`/`PHASE`/`PREB1_PHI_OFF`/… and `diagnostics[0].write_dir`). To scan, edit those keys (e.g. `PREB1_KW` + a per-case `write_dir: injector/diags/P<kw>_zc`) and re-run `injector.run(plots=False)` per point; a single `injector.plot()` aggregates every `diags/P*` directory alongside the default `diags/main` (see `injector/README.md`). The faithful default is the 8 kW / 10 kW two-cavity point — the scan is exploration only. **Caveat:** a hard Preb-1 power scan desyncs the Preb-2 phase reference (Preb-2 is timed from the injection β + Preb-1's faithful kick); a hardened-Preb-1 study needs a two-pass run (see `injector/README.md`).
...
- `<stage>.yaml` — the **static lume-warpx config** (grid/solver/simulation/fields/species/diagnostics) plus a non-WarpX `params:` block of stage knobs. Holds **every constant** for the four WarpX stages; the operating point + the Balanced performance profile are baked in as the shipped defaults. Edit this file to retune a WarpX stage (the `config()` knob API is bypassed for them — see *Stage API*).
- `*_sim.py` — the WarpX run, driven through **lume-warpx** (`from warpx import WarpX`, the `warpx.WarpX` LUME wrapper). Loads `<stage>.yaml`, reads its constants back via `w.get(...)`, computes the runtime-derived values (RF/solenoid time-function strings, `dt`, `max_steps`, diagnostic periods), and writes only those back via `w.set/update(...)`; the upstream beam is imported via `WarpX(initial_particles=...)` (a `ParticleGroup` → `FromInitialParticles`) or, for the gun's time-release, a one-particle `ParticleList` seed plus a `beforestep` `install_callback`. `w.run(progress="<stage>")` drives the tqdm bar; diagnostics land in the stage's `diags/`. (lume-warpx itself is the user's package — see *Reference Materials*.)
- `plot_*.py` — reads `diags/`, writes figures to `results/`. Generic phase-space / field-plane / trend figures can use lume-warpx's `WarpX.plot2D` / `plot_fields` / `plot1D`; physics-overlay figures (Child–Langmuir, fixed-z virtual screens, self-field maps) stay custom.
- `README.md` — the stage's physics, field map, operating point, and outputs.

Four in-process **shared modules** under `pipeline/` are the single source of truth that the stage scripts import instead of re-literaling (see `pipeline/README.md` → *Shared modules*): `pipeline/constants.py` (scipy-sourced `C_LIGHT`/`E_CHARGE`/`M_E`/`EPS0`/`K_B`/`MC2_EV` — no stage carries its own rest-energy/charge literal), `pipeline/emission.py` (`child_langmuir_current_density`, `thermal_velocity_sigma` — shared by the cathode sim and its plot overlays), `pipeline/fieldio.py` (the GDF→openPMD thetaMode parse + writer `load_cols`/`to_grid`/`pad_r`/`write_thetamode_series`, used by all three `build_*_field.py`; `to_grid(reverse_descending_z=True)` row-reverses a z-descending GDF so its odd `Er` is not negated), and `pipeline/beam_io.py` (the WarpX beam-handoff idioms `open_particle_series`/`make_particle_group`/`downsample`/`beam_kinematics`/`rf_time_functions`, shared by the gun, injector, and linac_sec1 `load_*_bunch`/handoff paths plus `impact_io`/`beam_gui` — openPMD series open+guard, γβ→eV/c `ParticleGroup` build, weighted downsample, mean v/KE, and the RF `warpx_E/B_time_function` strings).
...
| `gun/gun_sim.py` | `cathode/diags/particles` + `gun/gun_field/gun_E.h5` | `gun/diags` (+ `gun/diags/handoff` in timed mode) |
| `injector/injector_sim.py` | `gun/diags/handoff` if present (timed-release exit beam) else `gun/diags/particles` (legacy snapshot) + `injector/injector_field/{preb1_EB,preb2_EB,lens0a,sol0,lens0e}.h5` | `injector/diags/main` |
...
| `linac_rest/linac_rest_sim.py` | `linac_sec1/diags/main/particles` (last/exit dump, captured ~21 MeV core ≥ `MIN_KE_MEV`) + vendored `linac_rest/rfdata/rfdata4–7` (S-band TW field shape) | `linac_rest/diags/main` |
...
- The **gun beam representation** is `BEAM_RELEASE` (default `"timed"`): the CESR gun is gated by a **2 ns grid pulse** (`cathode_master.in twidth=2`, ~4× the ~0.47 ns gun transit), so the physical beam is a long, low-density quasi-DC stream (the original GPT deck emits it via `settdist(...,"t",...)`). `"timed"` releases the imported cathode macroparticles over `PULSE_WIDTH` with a per-step `installbeforestep` `add_particles` callback (the WarpX *write* path works in this RZ build — only the *read* accessor is broken); `"snapshot"` injects all 1 nC at t=0, which over-concentrates the charge and over-states space charge (the WarpX–GPT 150 kV-gun cross-code benchmark, `CornellMisc/.../bench/writeup`, put the beam-representation effect at ~28% on emittance; here the over-dense snapshot additionally scrapes ~19% of the beam as halo — measured timed≈100% vs snapshot≈81% transmission, same grid/charge). The released beam's ~0.4 m ballistic extent can't fit the gun domain in one snapshot, so `build_exit_handoff()` reconstructs the full exit beam by **id-tracking across the volumetric dumps** (same idiom as `pipeline/collimator.py`). CRITICAL gotcha (physics-review): the domain must extend a **field-free drift pad** (`ZPAD`) past the field map (`ZMAX_FIELD`; WarpX zero-fills the applied field beyond the map, verified) and the reconstruction must sample each id at its **first appearance in that field-free pad** (z≥`ZMAX_FIELD`) — a field-free ballistic drift to a common reference time preserves εn,x, but sampling/drifting a still-in-field particle (the naive last-in-domain dump) manufactures a spurious x–u correlation and inflates the handoff εn,x **~8×**. Run length is sized on the FIELD transit (not the padded domain) so the run stops while the beam is in the pad — draining the padded domain empties it and aborts the MLMG solve (`MLMG failed`). Non-pad-reaching ids are classified radial-`r=RMAX` loss vs un-flushed tail (~2%, finite run). The handoff εn,x is the **projected** emittance of the ~0.4 m instantaneous beam (head drifted longer than tail — physical, larger than a per-slice screen value). Writes `gun/diags/handoff` via `pipeline.impact_io.write_openpmd_particles`; the injector reads `handoff` when present. **Caveat:** the injector prebuncher phases were tuned to the compact snapshot handoff, so the longer timed beam shifts that operating point — re-validation is a follow-up (the LinacSim reconciliation backlog), the handoff wiring is done. The same benchmark also cross-validates the gun's electromagnetostatic (γ²) self-field, its dirichlet (not neumann) cathode-image z=0 plate, and motivated the finer **128×712** grid (near-cathode εn,x convergence, over the field-free-padded domain). The cathode stage needs no change — it is already a continuous-flux (time-release) emitter and non-relativistic.
...
| **lume-warpx** | the user's own package (`github.com/ryland-goldman/lume-warpx`; import name `warpx`) | LUME wrapper that the four WarpX stages drive: a static YAML (grid/solver/simulation/fields/species/diagnostics) → `WarpX(input_file=…, path=…)` → `.get/.set/.update` override API, `install_callback`, `initial_particles`→`FromInitialParticles`, `AppliedFromFile`→`LoadAppliedField`, and `run(progress=…)` with a tqdm bar; plus `plot2D`/`plot1D`/`plot_fields` helpers. Pinned `lume-warpx==1.0.1` in `requirements.txt`; develop against a local editable clone. |
...
| **lume-gpt** | `reference/lume-gpt Documentation/README.md` | Python wrapper for GPT (import name `gpt`): `GPT` class builds/runs decks, parses GDF output into `ParticleGroup` touts/screens, marker-comment autophasing (`gpt_phasing`), programmatic `Lattice`/field-map elements, distgen integration (`run_gpt_with_distgen`). Templates include the Cornell `cu_injector` front end. |
| **distgen** | `reference/distgen Documentation/README.md` | Declarative (YAML + pint units) initial-particle-distribution generator: `Generator` → `ParticleGroup`; cathode/time/free starts, large 1D/radial/2D dist catalog, ordered transforms, writers for GPT/ASTRA/openPMD/SIMION. The standard beam front end for lume-gpt decks. |
| **GPT_tools** | `reference/GPT_tools Documentation/README.md` | Adam Bartnik's analysis layer over lume-gpt: `gpt_plot`/`gpt_plot_gui` (ipywidgets) trend + dist plots, `ParticleGroupExtension` (core/slice emittance, actions), settings-driven run harness with auto-phasing (`run_gpt_with_settings`), screen postprocessing, xopt Pareto-front explorer. |
