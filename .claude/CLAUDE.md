...
self-consistent chain — each reads the previous stage's openPMD beam. Sections 1–3 of the linac are
**WarpX** (RZ); the **3→4 boundary** (the linac section-3 exit) is a clean handoff and the future
slot for an e⁺/e⁻ converter target (not modelled here); sections 4–8 are **Impact-T**.
...
physics notes). The three WarpX linac sections share **one** parametrized driver (`sim/linac1-3.py`,
section chosen by a CLI argument); the five Impact-T sections share `sim/linac4-8.py`.
...
- **No `config()`/profile API.** Each WarpX stage reads `config/<stage>.yaml`; the Impact-T stage
  reads `config/linac4-8.yaml`. Retune by editing the YAML. The shipped values are the single
  Balanced operating point.
- **Frozen RF setpoints.** The linac RF crest phases and field scales are **hardcoded** in
  `config/linac2.yaml`, `linac3.yaml`, `linac4-8.yaml` (`CREST_PHASE_DEG`/`FIELD_SCALE`, and the
  per-section `crest_phase_deg`/`field_scale` table). They were derived once from the beam and the
  drivers simply read+apply them — there is no runtime crest-finding or calibration loop. **If you
  change an upstream knob that shifts the beam, the affected setpoint must be re-derived** (load the
  upstream exit dump + the on-axis field, run the crest math once, paste the result). The Impact-T
  per-section crest is *absolute* (`theta0_deg`), so it is only valid for the deck geometry it was
  derived on — keep the deck length (≈28.974 m, real-length zero-K1 inter-section quads) unchanged.
- **Space charge / speed.** The self-field MLMG Poisson solve dominates WarpX runtime. The
  relativistic linac sections 2–3 run with `warpx_do_not_deposit: true` (**SC off**): at γ≳45 the
...
- **Run one stage off existing upstream output:** each stage reads the previous stage's openPMD
  ...
  sim/linac1-3.py 2` directly if `logs/diags/linac1-3/sec1/` already exists.
...
- `buildfields.py` — GDF→openPMD thetaMode field builders (`load_cols`/`to_grid`/`pad_r`/
  ...
  Reads `fieldmaps/gdf/`, writes `fieldmaps/h5/`. The single SLAC map is shared by linac 1–3.
...
lume-impact (`import impact`) for `linac4-8`. Each: `prepare_env()` → build field → import the
...
+ `injection_summary.json`. `sim/linac1-3.py <N>` branches on the section: N=1 is the capture stage
(iris scrape, `scale=√(POWER/RF_NORM)`, on-crest phase); N=2,3 read the previous exit's captured
core and apply the frozen `FIELD_SCALE`/`CREST_PHASE_DEG`.
...
| `sim/linac1-3.py 1` | `logs/diags/injector/main/particles` (dump near z≈2.03 m, iris-scraped) + `fieldmaps/h5/linac_rf{1,2}.h5` | `logs/diags/linac1-3/sec1/main` |
| `sim/linac1-3.py 2` | `logs/diags/linac1-3/sec1/main/particles` (captured core) + the shared SLAC maps | `logs/diags/linac1-3/sec2/main` |
| `sim/linac1-3.py 3` | `logs/diags/linac1-3/sec2/main/particles` (captured core) | `logs/diags/linac1-3/sec3/main` (the 3→4 boundary) |
| `sim/linac4-8.py` | `logs/diags/linac1-3/sec3/main/particles` (≥ `MIN_KE_MEV` core) + `fieldmaps/rfdata/rfdata4–7` | `logs/diags/linac4-8/main` |
...
- **Linac 1–3** share the SLAC quadrature maps (Re/Im halves of *one* TW section, summed at 90°);
  ...
  `injection_summary.json` (`z_handoff_m` for sec1, `z_inject_lab_m` for sec2/3).
- **Linac 4–8** (Impact-T): no field maps exist — the five S-band TW sections reuse the vendored
  `rfdata4–7` shape as a 4-line `solrf` superposition (+0/+30/+90/+0, body `/sin(β₀d)`), all physics
  in the frozen per-section `field_scale`; `theta0_deg` is **absolute** so each section's crest is a
  distinct frozen number (and depends on deck geometry — keep the real-length quads); SC off, quads
  off (K1=0); transmission measured from the **macro count before** re-imposing charge;
...
- **lume-impact** (import name `impact`): `Impact` class for the `linac4-8` deck (solrf elements,
