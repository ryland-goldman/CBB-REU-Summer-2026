...
those back and overrides only the runtime-computed values (flux, thermal velocity, dt,
diagnostic periods). Over-injects at 2×J_CL and lets WarpX's self-consistent fields build a
virtual cathode that self-limits the transmitted current to J_CL. See docs/cathode.md for
physics, parameters, and gotchas.

main() runs ONLY the simulation; sim/plot/cathode.py produces the figures.
"""
