...
Usage:
    import linac_sec1
    linac_sec1.config(POWER_MW=15, PHASE_DEG=0, I_SOL=1000)   # optional overrides
    linac_sec1.run()                                          # build field + sim + plots
    linac_sec1.run(plots=False)                               # build + sim only
    linac_sec1.plot()                                         # plots only

The default operating point (top of `linac_sec1_sim.py`) is the on-crest, focused case
(`PHASE_DEG=0`, `I_SOL=1000`); a bare `run()` reproduces the headline result.
