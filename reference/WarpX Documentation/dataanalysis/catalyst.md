...
First, we build
[Conduit](https://llnl-conduit.readthedocs.io/en/latest/building.html) and then
build [Catalyst 2](https://catalyst-in-situ.readthedocs.io/en/latest/build_and_install.html)
...
[here](https://github.com/Kitware/ParaView/blob/master/Documentation/dev/build.md) .
A representative set of options for a headless ParaView installation is provided
[here](https://gitlab.kitware.com/christos.tsolakis/catalyst-amrex-docker-images/-/blob/ci-catalyst-amrex-warpx-20240701/docker/ubuntu22_04/install_paraview.sh)
...
The supported parameters are part of the [FullDiagnostics](../usage/parameters.md#running-cpp-parameters-diagnostics) with [`<diag_name>.format`](../usage/parameters.md#diag_name-.format) parameter set to `catalyst`.
...
The following script, `simple_catalyst_pipeline.py`, automatically detects the type of data for both the mesh and particles, then creates an extractor for them. In most
...
For more information see the documentation for catalyst replay [here](https://catalyst-in-situ.readthedocs.io/en/latest/catalyst_replay.html) .
