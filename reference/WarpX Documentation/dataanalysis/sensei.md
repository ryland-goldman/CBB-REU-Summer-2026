...
<a id="sensei-arch"></a>
![image](https://data.kitware.com/api/v1/item/5c06cd538d777f2179d4aaca/download)
...
[Fig. 29](#sensei-arch). On the left side of the figure AMReX produces data, the
...
set automatically using the `sensei_config` command line tool that should
...
Typically, the `sensei_config` tool is in the users PATH after loading
the desired SENSEI module. After configuring the build environment with
`sensei_config`, proceed as usual.
...
|-------------------|--------------------------------------------------------------------------------------|-----------|
| `insitu.int`      | turns in situ processing on or off and controls how<br/>often data is processed.     |         0 |
| `insitu.start`    | controls when in situ processing starts.                                             |         0 |
| `insitu.config`   | points to the SENSEI XML file which selects and<br/>configures the desired back end. |           |
| `insitu.pin_mesh` | when 1 lower left corner of the mesh is pinned to<br/>0.,0.,0.                       |         0 |

A typical use case is to enabled SENSEI by setting `insitu.int` to be
greater than 1, and `insitu.config` to point SENSEI to an XML file that
...
with WarpX to render 10 iso surfaces, shown in [Fig. 30](#lpa-visit), using
VisIt Libsim.
...
<a id="lpa-visit"></a>
![image](https://data.kitware.com/api/v1/item/5c06b4b18d777f2179d4784c/download)

The same run and visualization was repeated using ParaView Catalyst, shown in
[Fig. 31](#lpa-pv), by providing the following XML configuration.
...
<a id="lpa-pv"></a>
![image](https://data.kitware.com/api/v1/item/5c05b6388d777f2179d207ae/download)
...
`.visit` file that VisIt can open will be generated.
...
SENSEI is hosted on Kitware’s Gitlab site at [https://gitlab.kitware.com/sensei/sensei](https://gitlab.kitware.com/sensei/sensei)
...
The SENSEI VM can be downloaded [here](https://data.kitware.com/api/v1/file/5be656368d777f21799ee5a6/download).
...
for `Initialize`, `Execute`, and `Finalize` phases of the run.
...
`Initialize` and `Finalize` are optional and will be called if
they are provided. `Execute` is required. SENSEI’s DataAdaptor API
...
The `script_file` attribute sets the file path to load the user’s Python
code from, and the `initialize_source` element contains Python code that
