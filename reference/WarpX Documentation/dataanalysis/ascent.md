...
After building and installing Ascent according to the instructions at [Building Ascent](https://ascent.readthedocs.io/en/latest/BuildingAscent.html), you can enable support for it in WarpX by changing the line
...
The supported parameters are part of the [FullDiagnostics](../usage/parameters.md#running-cpp-parameters-diagnostics) with [`<diag_name>.format`](../usage/parameters.md#diag_name-.format) parameter set to `ascent`.
...
[Ascent Actions](https://ascent.readthedocs.io/en/latest/Actions/index.html) provides an overview over all available analysis and visualization actions.
...
[Replay](https://ascent.readthedocs.io/en/latest/Utilities.html#getting-data-for-replay) is a utility that allows the user to replay a simulation from aforementioned files and rendering them with Ascent.
...
A visualization of the electric field component $E_x$ (variable: `Ex`) with a contour plot and with added particles can be obtained with the following Ascent Action.
...
There are more [Ascent Actions examples available](https://ascent.readthedocs.io/en/latest/Actions/Examples.html#yaml-examples) for you to play.
...
Second, copy the Jupyter Notebook file [`ascent_replay_warpx.ipynb`](../../../Tools/Ascent/ascent_replay_warpx.ipynb) into the simulation output directory.
...
Now, access Jupyter Lab via: [http://localhost:8888/lab](http://localhost:8888/lab) (password: `learn`).
