...
Load balancing can be activated in WarpX by setting input parameters, see the [parallelization input parameter section](../usage/parameters.md#running-cpp-parameters-parallelization).

<a id="developers-profiling-tiny-profiler"></a>
...
For more detailed information please visit the [AMReX profiling documentation](https://amrex-codes.github.io/amrex/docs_html/AMReX_Profiling_Tools_Chapter.html).
There is a script located [here](https://github.com/AMReX-Codes/amrex/tree/development/Tools/TinyProfileParser) that parses the Tiny Profiler output and generates a JSON file that can be used with [Hatchet](https://hatchet.readthedocs.io/en/latest/) in order to analyze performance.
...
- Compile WarpX with full profiler support:
  ...
  > #### WARNING
  > Please note that the [AMReX build options](https://amrex-codes.github.io/amrex/docs_html/BuildingAMReX.html#customization-options) for `AMReX_TINY_PROFILE` (our default: `ON`) and full profiling traces via `AMReX_BASE_PROFILE` are mutually exclusive.
...
<a id="developers-profiling-nsight-systems"></a>

## Nvidia Nsight-Systems

[Vendor homepage](https://developer.nvidia.com/nsight-systems) and [product manual](https://docs.nvidia.com/nsight-systems/).
...
You can either run this on an interactive node or use the Slurm batch script header [documented here](../install/hpc/perlmutter.md#running-cpp-perlmutter).
...
> Example on how to create traces on a multi-GPU system that uses the `jsrun` scheduler (e.g., [OLCF’s Summit system](https://docs.olcf.ornl.gov/systems/summit_user_guide.html#optimizing-and-profiling)):
```bash
...
* [`warpx.numprocs = ...`](../usage/parameters.md#warpx.numprocs): make the run short, reasonably small, and run only a few steps
...
[Vendor homepage](https://developer.nvidia.com/nsight-compute) and [product manual](https://docs.nvidia.com/nsight-compute/).
...
#### NOTE
Note the trailing `/` at the end of the `--nvtx-include` filter ([reference](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html?highlight=nvtx%2520include#nvtx-filtering)).
...
* `dcgmi profile --pause` other profiling tools can’t be collecting data,
  [see this Q&A](https://forums.developer.nvidia.com/t/profiling-failed-because-a-driver-resource-was-unavailable/205435).
...
significant. For full information, see the Nvidia’s documentation on [NVTX filtering](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html#nvtx-filtering) .
