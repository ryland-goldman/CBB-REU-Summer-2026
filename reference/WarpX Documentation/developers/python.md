<a id="development-python"></a>

# Processing PICMI Input Options

The input parameters in a WarpX PICMI file are processed in two layers.
The first layer is the Python level API, which mirrors the [C++ application input structure](../usage/parameters.md#running-cpp-parameters); the second is the translation from the PICMI input to the equivalent [app (AMReX) input file parameters](../usage/parameters.md#running-cpp-parameters).

The two layers are described below.

## Input parameters

In a C++ input file, each of the parameters has a prefix, for example `geometry` in [`geometry.prob_lo`](../usage/parameters.md#geometry.prob_lo-hi).
...
assigning attributes of class instances, for example [`geometry.dims = 3`](../usage/parameters.md#geometry.dims).
...
For example, for the particle species, an instance is created for each species listed in [`particles.species_names`](../usage/parameters.md#particles.species_names).
...
There are two input parameters that do not have prefixes, [`max_step`](../usage/parameters.md#max_step) and [`stop_time`](../usage/parameters.md#stop_time).
...
The two parameters that do not have a prefix, [`max_step`](../usage/parameters.md#max_step) and [`stop_time`](../usage/parameters.md#stop_time), are passed into the `warpx` method as keyword
arguments.
