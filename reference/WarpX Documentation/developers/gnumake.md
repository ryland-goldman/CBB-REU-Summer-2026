<a id="developers-gnumake"></a>

# GNUmake Build System (Legacy)

[CMake](../install/cmake.md#install-build-cmake) is our primary build system.
In this section, we describe our legacy build scripts - do not use them unless you used them before.

WarpX is built on [AMReX](https://amrex-codes.github.io), which also provides support for a Linux-centric set of build scripts implemented in GNUmake.
...
Please see the CMake-based [developer section](../install/cmake.md#install-build-cmake) instead.
...
> * `MPI_THREAD_MULTIPLE=TRUE` or `FALSE`: Whether to initialize MPI with thread multiple support. Required to use asynchronous IO with more than [`amrex.async_out_nfiles`](../usage/parameters.md#amrex.async_out_nfiles) (by default, 64) MPI tasks.
>   Please see [data formats](../dataanalysis/formats.md#dataanalysis-formats) for more information.
...
For a description of these different options, see the [corresponding page](https://amrex-codes.github.io/amrex/docs_html/BuildingAMReX.html) in the AMReX documentation.
...
* [Building WarpX with support for openPMD output](gnumake/openpmd.md)
* [Building the spectral solver](gnumake/spectral.md)
* [Building WarpX to use RZ geometry](gnumake/rzgeometry.md)
* [Building WarpX with GPU support (Linux only)](gnumake/gpu_local.md)
* [Installing WarpX as a Python package](gnumake/python.md)
* [Building WarpX with Spack](gnumake/spack.md)
