...
Due the AMR strategy used in WarpX (see section [Theory: AMR](../theory/amr.md#theory-amr) for a complete description), each field on a given refinement level `lev` (except for the coarsest `0`) is defined on:
...
<a id="developers-fields-names"></a>
...
#### WARNING
doxygenenum: Cannot find file: /Users/rylandgoldman/Downloads/warpx-development/Docs/doxyxml/index.xml
...
The innermost step `// Apply field solver on the FAB` could be done with 3 nested `for` loops for the 3 dimensions (in 3D). However, for portability reasons (see section [Developers: Portability](portability.md#developers-portability)), this is done in two steps: (i) extract AMReX data structures into plain-old-data simple structures, and (ii) call a general `ParallelFor` function (translated into nested loops on CPU or a kernel launch on GPU, for instance):
