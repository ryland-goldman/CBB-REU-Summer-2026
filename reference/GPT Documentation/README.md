...
Customizing GPT is the subject of a separate manual, the GPT Programmer's Reference. This markdown-based manual was automatically generated from a PDF version and then cleaned up: parameter/flag definitions are rendered as `**`name`**` — description lists, and key tables (operator precedence, built-in constants/functions in §4.1, the space-charge summary in §1.5) were rebuilt as markdown tables from the PDF. Some equations and figures did not survive the PDF extraction — those spots are marked *[equation/table lost in PDF extraction — see UserManual.pdf §x.y]*, and any remaining garbled inline math should likewise be checked against [UserManual.pdf](UserManual.pdf) (the authoritative copy).
