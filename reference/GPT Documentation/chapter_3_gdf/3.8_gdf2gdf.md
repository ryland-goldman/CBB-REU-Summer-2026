...
- **`-i`** — Ignore recoverable errors and continue.

- **`-s`** — Sort top-level group.

- **`-t group`** — Top-level group.

- **`-o outfile`** — Write output to `outfile`.

- **`-a outfile`** — Append output to `outfile`.

- **`infile[s]`** — Names of all the GDF files to be concatenated

- **`value[s]`** — Optional values for the top-level group.
