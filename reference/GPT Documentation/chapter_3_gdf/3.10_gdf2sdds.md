...
-v
```
Verbose mode, instructs the program to write additional information during processing.

```
-a
```
Writes an ASCII SDDS file. When not specified, a binary file is written. Before use, please see the note below about precision.
...
infile
```
Filename of the GDF file to convert to ASCII. If not specified, the program reads the GDF file from `stdin`. To read from `stdin` when `arrays` are specified, a dash (‘`-`’) must be specified.

```
arrays
```
The names of the arrays/parameters to convert. If not specified, all GDF information is converted to the SDDS format.
