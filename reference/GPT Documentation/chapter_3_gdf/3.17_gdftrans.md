...
- **`-s array`** — Sort by `array`. Default set to array `"ID"`.

- **`-o outfile`** — Write output to outfile.

- **`-a outfile`** — Append output to outfile.

- **`infile`** — Name of the file to perform the calculations on. When GDFTRANS has to read from `stdin`, a dash (`-`) must be specified explicitly for infile.

- **`group_by`** — The name of the variable used to group the data.

- **`arrays ...`** — The name(s) of one or more arrays to convert. Typicaly these are `x` `y` `z` `Bx` `By` `Bz` and `G`.
