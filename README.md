...
(SCL diode)  (~149 keV)  (2 prebunchers + 3 solenoids)  (~21 MeV captured)  (sections 2–8, ~305 MeV, Impact-T)
```
...
`cathode.config(V_anode=30)` etc. to override the module-level parameters before calling `.run()`
...
| **4. Linac Sec 1** | [`linac_sec1/`](linac_sec1/README.md) | SLAC-design 3 m, 2π/3 traveling-wave accelerating section (RZ). Reads the injector's focused beam at the z ≈ 2.03 m handoff and applies the multi-plane 9.547 mm iris scrape at injection; no in-stage solenoid (focusing is upstream now). At the faithful 11 MW point capture is ~10% of true injected to ⟨KE⟩ ≈ 21 MeV — a conservative (γ²) lower bound, tune-sensitive to the upstream lens currents. |
| **5. Linac Rest** | [`linac_rest/`](linac_rest/README.md) | Cornell linac **sections 2–8** (CEA 2/3/4/5 + CU 3/4/5) — seven S-band traveling-wave sections chained into one **Impact-T** deck (via lume-impact, not WarpX). Reads `linac_sec1`'s captured ~21 MeV exit beam, keeps the β > 0.999 core (a model-validity cut), and accelerates on-crest to ≈305 MeV at 11 MW (304.9 survivors through the real bore). No field maps (generic constant-gradient TW, reusing the lume-impact `rfdata4–7` shape); space charge off by default; quads OFF for the headline. The e+ compressor (CU 2) is out of scope. |
