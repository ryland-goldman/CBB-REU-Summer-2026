...
def _derive_outdir(power_kw, phase):
    # `:g` keeps whole kW compact (800.0 -> "P800_zc", matching the shipped
    # config) while preserving fractional operating points (160.5 -> "P160.5_zc")
    # so a power scan over non-integer kW can't collide distinct cases into one
    # OUTDIR. Integer-valued int() would truncate 160.5 and 160.9 to the same dir.
    return f"prebuncher/diags/P{power_kw:g}_{phase}"
...
def resolve_outdir():
    ...
    OUTDIR explicitly set via config() wins; otherwise derive from POWER_KW/PHASE
    ...
    if p.get("OUTDIR"):
        return p["OUTDIR"]
    return _derive_outdir(p.get("POWER_KW", DEFAULT_POWER_KW),
