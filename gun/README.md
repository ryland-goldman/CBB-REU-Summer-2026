...
Generated by `plot_gun.py` in three layers — lume-warpx's plotting helpers, the shared custom
figures in `pipeline/plot_extras.py`, and the stage-specific rich figures (raw openPMD):
...
7. **`beamsize_vs_t.png`** — `plot1D("t","sigma_x")`: the transverse envelope σ_x across the gun.
8. **`energy_spectrum.png`** — `plot_extras.energy_spectrum`: charge-weighted exit KE histogram (⟨E⟩/σ_E) — the ~150 keV energy spread.
9. **`current_profile.png`** — `plot_extras.current_profile`: longitudinal current I(z) of the time-released 2 ns pulse stream.
10. **`beam_spot_xy.png`** — `plot_extras.beam_spot`: the transverse x–y spot (RZ-reconstructed y) with marginals — a roundness check.
11. **`gun_field.png`** — the on-axis applied E_z(z) and the potential it implies (cathode → exit), from `gun_E.h5`.
12. **`beam_rz.png`** — the beam shape in r–z at launch / mid-gun / exit.
13. **`energy_gain.png`** — mean and max KE vs z on **fixed-z virtual screens** (`beam_metrics.screen_profile`): a local-in-z energy gain saturating at the 150 keV gun voltage. The screen reconstruction (id-track each particle and interpolate to every z-plane it crosses) avoids the bin-to-bin ε_n,x jitter a z-histogram of the timed quasi-DC stream would produce — see *Beam representation*.
14. **`beam_envelope.png`** — σ_x and ε_n,x vs z on the same virtual screens: the transverse focusing (σ_x ~4→1.3 mm) and emittance evolution.
15. **`space_charge.png`** — the beam **self-field** ρ(r,z) and φ(r,z) (the dumped self-consistent fields) near launch, separate from the applied gun field.
