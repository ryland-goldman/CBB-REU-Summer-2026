"""
Reusable custom beam figures shared by the WarpX stage plot scripts
(cathode/gun/injector `plot_*.py`). Each helper takes a `pmd_beamphysics.ParticleGroup`
(one diagnostic dump, e.g. the exit beam) and returns a matplotlib Figure.

These are the physics-overlay figures that lume-warpx's generic `plot2D`/`plot1D`/`plot_fields`
don't cover: the energy spectrum, the longitudinal current profile, the transverse spot, and
the slice energy chirp. See each stage README for what the figure shows for that stage.
"""
...
def energy_spectrum(pg, n_bins=80, use_ke=True):
    ...
    ax.axvline(mean, color="k", ls="--", lw=1,
               label=f"⟨E⟩ = {mean:.1f} keV\nσ_E = {std*1e3:.0f} eV  ({100*std/mean:.2f}%)")
...
def current_profile(pg, n_bins=80):
    ...
    Keyed on z (not t): a WarpX diagnostic is an instantaneous snapshot (σ_t ≈ 0), so the
    bunch structure lives in z. Each particle contributes weight·v_z/dz to the local current
    (per-particle v_z, not a single mean — at the cathode the velocity varies strongly with z).
    """
    from pipeline.constants import C_LIGHT
    ...
    current = counts / dz                                            # I = Σ(w·v_z)/dz [A]
...
def beam_spot(pg, bins=160):
    ...
    fig = pg.plot("x", "y", bins=bins, return_figure=True)   # joint plot: marginals + scatter
